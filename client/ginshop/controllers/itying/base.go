...
func (con BaseController) Render(c *gin.Context, tpl string, data map[string]interface{}) {
	...
	if hasTopNavList, err := models.CacheDb.Get("topNavList", &topNavList); !hasTopNavList && err == nil {
	...
	if hasGoodsCateList, err := models.CacheDb.Get("goodsCateList", &goodsCateList); !hasGoodsCateList && err == nil {
	...
	if hasMiddleNavList, err := models.CacheDb.Get("middleNavList", &middleNavList); !hasMiddleNavList && err == nil {
